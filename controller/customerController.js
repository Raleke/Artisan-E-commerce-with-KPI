...
const jobSearch = async (req, res) => {
  try {
    ...
    if (results.length > 0) {
      ...
      res
        .status(404)
        .json({ results: [], searchParam, message: "No artisans found." });
    }
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({ msg: "An error occurred while searching for artisans." });
  }
};
...
const hireArtisan = async (req, res) => {
  try {
    ...
    if (!customer || !artisan)
      return res.status(404).json({ msg: "Customer or Artisan not found." });

    customer.jobRequests.push({
      artisanId,
      jobCategory,
      jobDescription,
      budget,
